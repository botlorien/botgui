...
from botgui.exceptions import *
from botgui.cript import Cript
import botgui.globals_gui as gb
...
class Interface:
    """
    A class that provides an interface for entering and storing encrypted 
    credentials.
    ...
    def __init__(self, appname: str = 'BotLórien', user: str = '', env=True):
        """
        Constructor method.
        Initializes the SswInterface object with several tkinter widgets and 
        an instance of the SswCript class.
        """
        super().__init__()
        ...
        self.img_frame = None
        self.counter_img = 0
        self.home_options_textbox_frame = None
        self.home_options_buttons_frame = None
        self.name_bot = self.create_file_txt('', 'config_bot_name', 'config')

        self.path_credentials = os.path.abspath(
            os.path.join(os.getcwd(), f'parameters/{self.appname}_credentials.bot')
            )
    ...
    def ui_show_imgs(self, text, folder_imgs, list_name_img_buttons, list_func_img_buttons, **kwargs):
        ...
        if all([self.home_options_textbox_frame, self.home_options_buttons_frame]):
    ...
    def _insert_value_textbox(self, text: str):
        self.textbox_config.insert("0.0", text.strip())
